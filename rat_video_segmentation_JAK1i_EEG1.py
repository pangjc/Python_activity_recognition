...
# Doing a little bit customization for considering EEG connection wires
# 04/20/2015
class Window(tk.Frame):
    ...
    def client_load(self):
        ...
        def segmentation_frame(frame0,frame_base,maskRegion,kernel):
            ...
            ret,fgmask0 = cv2.threshold(diff,90,255,cv2.THRESH_BINARY)
            ret,fgmask1 = cv2.threshold(frame_base,240,255,cv2.THRESH_BINARY_INV) # for lights points
            fgmask0 = fgmask0*fgmask1
            
            #ret,fgmask0 = cv2.threshold(diff,0,255,cv2.THRESH_BINARY+cv2.THRESH_OTSU)
