...
# This script aims to process videos based on James' setup
...
class Window(tk.Frame):
    ...
    def client_load(self):
        # Define a slider to select base frame
        def onChange(trackbarValue):
            cap.set(cv2.cv.CV_CAP_PROP_POS_FRAMES,trackbarValue)
            ret,img = cap.read()
            cv2.imshow("Select base frame", img)
            pass
        ...
        #print fps
        fps = 15.0
        trackBarPanel = np.zeros((300,512,3),np.uint8)
        
        cv2.namedWindow('Select the starting frame')
        cv2.createTrackbar( 'frame number', 'Select the starting frame', 0, length, onChange)
        start = 0
        while (1):
            cv2.imshow('Select the starting frame',trackBarPanel)   
            k = cv2.waitKey(1) & 0xFF
            if k == 27:
                break
        
            start = cv2.getTrackbarPos('frame number','Select the starting frame')
